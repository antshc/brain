...
`ralph-init` resolves the Source Repository with the same fail-closed topology contract as worktree setup, infers and confirms one primary senior-developer persona from repository evidence, and writes its delimited expertise and working-style block directly into Codey and Chorey. `ralph-init` also creates `VERIFY.md`; `ralph-verify` reads that required guidance directly and has no fallback path.