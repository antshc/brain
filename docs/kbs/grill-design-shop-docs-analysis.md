# Analysis: `grill-design` skill × shop-docs (dynamic Concept/ADR coverage)

Scope: does `grill-design` (→ `grilling` + `domain-modeling`) reliably surface every *relevant*
Concept/ADR during feature design on shop-docs, given that the Concept/ADR list is dynamic and
...
`manage-docs/ARCHITECTURE-FORMAT.md`, `CONCEPT-FORMAT.md`, `ADR-FORMAT.md`. For shop-docs this
means the 9-row Concepts table and 5-row ADR table in `shop-docs/ARCHITECTURE.md`.
...
- **Two-tier retrieval** (index scan → open-full-only-if-relevant) keeps context budget flat as
  `docs/concepts/`/`docs/adr/` grow — relevant since shop-docs already has 14 records.
...
- **Test-category challenge** ties into shop-docs' own `Testing Strategy`/Concept 0004, not a
...
1. **Relevance test is keyword/scope-gated, and that's circular for the things it should catch.**
   ...
   "order metadata," or "bulk order," Concepts/ADRs like 0009 (DynamoDB placement) or 0005/0006 (order
...
2. **No distinction between "backbone, always-applies" Concepts and "conditional" ones.** By
   ...
   should be unconditionally in-scope for any shop-docs feature touching code structure, not
   keyword-matched into relevance like an ADR.
3. **The `Cornerstone`/`Local` ADR tag defined in `ARCHITECTURE-FORMAT.md` is unused in
   shop-docs.** The format says ADRs should be tagged Cornerstone (affects multiple building
   blocks/structure) vs Local, but shop-docs' actual table is only `# | Decision | Summary` — no
...
## Verification against shop-docs' actual list

Walking the 9 Concepts + 5 ADRs in `shop-docs/ARCHITECTURE.md`:
...
- **At risk of being skipped** unless the conversation happens to name the right term: 0009
  (DynamoDB placement), 0007 (config tweaks), 0006 (bulk order ops), ADR 0004 (order retention), ADR
  0005/0006 (order metadata/customer capture), ADR 0008 (ScaleStorefronts) — all narrow/localized
  and easy to miss if the feature touches them implicitly (e.g. adding an order field without
...
2. **Adopt the `Cornerstone`/`Local` ADR tag** in shop-docs' `ARCHITECTURE.md` (already defined in
