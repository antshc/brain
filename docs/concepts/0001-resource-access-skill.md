...
- Direct use of the underlying infra tool is still allowed. When a caller does invoke the skill, it must do so via the skill's documented actions, using the style `run` / `via` `` `/{{skillName}}` `` action **{{ActionName}}** — never by inlining the underlying infra command in place of the skill reference.
