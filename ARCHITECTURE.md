...
| [0002](docs/adr/0002-droid-is-agnostic.md) | Droid is run-location- and technology-agnostic | `droid` workspace behavior, `droid` command execution, `.harness.env`/`resolve-harness` resolution, agent/skill naming, `droid-feedback` fallback behavior, adding language/toolchain-specific wording to the agent or its skills | Droid runs in the directory from which it is launched, has no `WORKTREE_PATH` interface, and stays agnostic about whether that directory is a worktree. It resolves `CODE.md`/`VERIFY.md`/`GOTCHAS.md` from its current directory, falling back to discovering the Harness Root via `resolve-harness`/`.harness.env` when they aren't present there. The agent and its `droid-*` skills carry no language/toolchain-specific knowledge; all technology specifics live only in those per-repo files, and the `droid-feedback` fallback discovers the toolchain via README + code exploration instead of hardcoding one. |
