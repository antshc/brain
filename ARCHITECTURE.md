...
| [0004](docs/adr/0004-trigger-indexer-is-resource-agnostic.md) | Trigger-indexer is resource-agnostic | `trigger-indexer` skill changes, adding a new indexed record type, hardcoding a file/section/directory into `trigger-indexer` | `trigger-indexer` carries no hardcoded index file, section, or record directory; `{{indexFile}}`, `{{indexSection}}`, `{{recordDirectory}}`/`{{recordPath}}` are always supplied by the caller in context at invoke time, so any future indexed record type reuses the same actions unchanged. |
...
| [0003](docs/concepts/0003-trigger-indexer.md) | Trigger Indexer | adding a new index-table row type, index table drifting out of sync with its records, a caller re-implementing scan/match logic inline | Centralizes scan/match (read) and add/supersede/retire (write) logic for any markdown table with a Trigger condition column behind one skill, so every caller gets identical, deterministic verdicts and every record change updates its index row in the same change. |
