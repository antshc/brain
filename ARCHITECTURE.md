...
| [0002](docs/concepts/0002-ledger.md) | Ledger | session tracking of opened Concept/ADR/service records, touched-surface accumulation, staging decisions and assumptions that are not yet durable, avoiding duplicate index re-scans, repeated file reads, broad codebase sweep, choosing between subagent delegation and a direct read, re-reading a document already in context | A session-scoped record, persisted via the memory tool at `/memories/session/domain-model-ledger.md`, of every record opened so far, the surface touched so far, and every decision/assumption taken so far — checked before any re-scope decision or conflict classification instead of relying on recall over a long context window. Also carries the session's context-economy rules: delegate broad sweeps, anchor-precision reads, re-fetch discipline. |
