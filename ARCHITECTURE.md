...
|---|----------|-------------------|---------|
| [0003](docs/adr/0003-ralph-owns-a-portable-agent-pipeline.md) | Ralph owns a portable agent pipeline | Ralph agent-handoff logic, Codey or Chorey orchestration, Ralph plugin agent packaging, agent workspace behavior, invocation-directory guidance, agent/skill naming, generated persona blocks, required `VERIFY.md` guidance, or source-repository topology | Ralph packages and directly orchestrates Codey followed by Chorey as one installed workflow. The agents use their invocation directory as the workspace and never receive or discover a `WORKTREE_PATH`. `ralph-init` resolves the Source Repository, confirms a primary persona, writes matching persona blocks into both agents, and creates required `VERIFY.md` guidance without a verification fallback. |
