---
description: How to write a human-facing README.md for a skill, published as a repo docs page — adapted from mattpocock/skills' writing-docs.md.
applyTo: "**/skills/**/README.md,docs/skills/**/*.md"
---
# Writing docs pages

Every skill under a plugin bucket in `skills/<plugin>/<skill-name>/` has a human-facing **docs page** at `docs/skills/<plugin>/<skill-name>.md`. The docs tree mirrors the plugin bucket folders under `skills/` (`atl`, `aws-platform`, `azure-platform`, `crew`, `engineering`, `harness`, `ralph`, `review`, `wf`). It is published at `https://github.com/antshc/brain/blob/main/docs/skills/<plugin>/<skill-name>.md`; the path always carries the plugin, unlike a flat slug, because two plugins can otherwise reuse the same skill name. The page is not the skill and not a copy of `SKILL.md`. Only skills registered in a plugin's `plugin.json` (or, for `engineering`, listed in its `README.md`) are promoted; skills under `_backup/`, `_in-progress/`, or otherwise unregistered ship no docs page.

Most of these skills are **user-invoked** (`disable-model-invocation: true`): the agent will never fire them for you, so *you* are the index that has to remember they exist and when to reach for them. That memory is **cognitive load**. The job of a docs page is to relieve it: to orient one reader around one skill so they can hold it in their head, know when to reach for it, and see where it sits in the system. The pages are collectively a distributed router; each is a node.

Act whenever a promoted skill is added, renamed, or has its behaviour changed: create or re-sync its docs page. A rename moves the file too (`docs/skills/<plugin>/<old>.md` → `docs/skills/<plugin>/<new>.md`), because the published URL tracks the name; a skill that moves between plugin buckets moves its docs file to the matching folder. Skills that move *out* of `_backup/` or `_in-progress/` into a registered plugin bucket gain a page; one moving the other way loses it.

Because these pages are published as repo files, **every link is absolute**: never a repo-relative path. A link to another skill points at `https://github.com/antshc/brain/blob/main/docs/skills/<plugin>/<name>.md`; a link into the repo points at its full `https://github.com/antshc/brain/...` URL. A relative link that works in the repo breaks once published.
...
**A page carries no install commands.** [README.md](https://github.com/antshc/brain/blob/main/README.md) is the single source for `copilot plugin install` / `gh skill install` commands, organised per plugin bundle. A page that also writes them out shows the reader the same command twice, and the two copies drift: the hand-written pair on every page went stale against the README beside it. Install wording is a property of the top-level README, not of the page — link to its Installation section instead of repeating a command.
...
- **Trigger boundary.** The index entry: "reach for this when …". Where the skill is confusable with a sibling, add the other half: "for <X> instead, use [<sibling>](https://github.com/antshc/brain/blob/main/docs/skills/<plugin>/<sibling>.md)."
...
- **This repo's issues.** `gh issue list --repo antshc/brain --search "<skill-name>" --state all`. A question filed twice is a question the page owes an answer to.
- **`CHANGELOG.md`.** Anything renamed, moved, or behaviourally changed generates a "where did it go?" that the page has to answer. Skip this bullet while the repo has no `CHANGELOG.md`.
...
- The page exists at `docs/skills/<plugin>/<name>.md`, and no stale page survives a rename or bucket move.
