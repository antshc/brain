...
Every skill under a plugin bucket in `skills/<plugin>/<skill-name>/` has a human-facing **docs page** at `docs/skills/<plugin>/<skill-name>.md`. The docs tree mirrors the plugin bucket folders under `skills/` (`atl`, `aws`, `crew`, `engineering`, `harness`, `ms`, `ralph`, `review`, `wf`). It is published at `https://github.com/antshc/brain/blob/main/docs/skills/<plugin>/<skill-name>.md`; the path always carries the plugin, unlike a flat slug, because two plugins can otherwise reuse the same skill name. The page is not the skill and not a copy of `SKILL.md`. Only skills registered in a plugin's `plugin.json` (or, for `engineering`, listed in its `README.md`) are promoted; skills under `_backup/`, `_in-progress/`, or otherwise unregistered ship no docs page.
