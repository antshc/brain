...
def build_prompt(threads: list[ReviewThread]) -> str:
    ...
    Serialises each ReviewThread to a dict with a 'prefix' string key so the
    Copilot agent receives the same JSON structure it previously expected.

    Args:
        threads: List of actionable ReviewThread domain objects.
