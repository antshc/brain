...
3. **Map broad** — grep/semantic-search route tables, command registrations, event subscribers, schedulers for candidate entry points. Collect candidates before reading any of them deeply.
...
5. **Rank by risk** — confirm in this order: does this path actually execute for the framed case (vs. a dead or superseded lookalike) → config/feature-flag branching → error and edge cases → async ordering → performance → naming.
...
**Done when** every step from entry point to effect carries a citation, the diagram accounts for every recorded Fact, and the remaining unknowns can't change the answer.
...
Write to `docs/ongoing/<slug>.md` unless the user names a location or the repo documents features elsewhere. Fill [feature-research-template.md](feature-research-template.md), obeying its `**Rules**` blocks and deleting every one of them from the result.
