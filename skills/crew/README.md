...
- [skills/init-crew/SKILL.md](skills/init-crew/SKILL.md) — manual, user-invoked bootstrap that scaffolds missing `CODE.md`/`VERIFY.md`/`CHORE.md`/`GOTCHAS.md` from templates into `$HARNESS_REPO_PATH/.crew/`. Never touches `.harness.env`, never migrates an existing `.droid/` folder. Not part of either agent's pipeline.
