...
def _heading_name(md_text: str, fence_start: int, index: int) -> str:
    preceding = md_text[:fence_start]
    ...
    return f"{index:02d}-{slugify(heading_text)}"
...
def extract_mermaid(md_text: str) -> tuple[str, list[dict]]:
    ...
    nearest preceding `<summary>` text.
    """
    diagrams: list[dict] = []
    index = 0
    ...
    def _replace(match: re.Match) -> str:
        ...
        if id_match:
            ...
            if diagram_id in ids_seen:
                raise ValueError(
                    f"duplicate diagram-id {diagram_id!r} on diagrams "
                    f"{ids_seen[diagram_id]} and {index} — every id must be unique per file"
                )
            ids_seen[diagram_id] = index
            code = DIAGRAM_ID_RE.sub("", code, count=1)
