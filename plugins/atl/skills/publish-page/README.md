...
| `ATLASSIAN_SWIMLANE_DRAWIO` | No | `false` (falsy: absent, empty, or anything other than `1`/`true`/`yes`) | Opts a `swimlane-beta` diagram into the native Mermaid-to-draw.io converter instead of falling back to a static PNG |
