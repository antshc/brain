...
class OrderedListNode(Node):
    ...
    def __init__(self, node_dict: Dict):
        ...
        self._start_order = self._attrs.get('order', 1)
        self._elements = list()
        for child_node in self._child_nodes:
            if child_node.type != NodeType.LIST_ITEM:
                print(f"WARNING '{NodeType.LIST_ITEM.value}' expected under orderedList; but '{child_node.type}' "
                      f"appeared")
                continue
