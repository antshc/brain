    ...
    def __init__(self, node: Node, depth: int = 0):
        ...
        for child in self._node.child_nodes:
            if child.type == NodeType.LIST_ITEM:
                self._child_presenters.append(ListItemPresenter(child, self._depth))

    def __str__(self):
        bulleted_list = list()
        indent = '  ' * self._depth
        for child_presenter in self._child_presenters:
            lines = str(child_presenter).split('\n')
            bulleted_list.append(f'{indent}+ {lines[0]}')
            for line in lines[1:]:
                bulleted_list.append(line)
...
class OrderedListPresenter(NodePresenter):
    ...
    def __init__(self, node: Node, depth: int = 0):
        self._depth = depth
        self._start_order = node.start_order if hasattr(node, 'start_order') else 1
        self._node = node
        self._child_presenters = list()
    ...
    def __str__(self):
        ordered_list = list()
        indent = '  ' * self._depth
        for idx, child_presenter in enumerate(self._child_presenters):
            number = self._start_order + idx
            lines = str(child_presenter).split('\n')
            ordered_list.append(f'{indent}{number}. {lines[0]}')
            for line in lines[1:]:
...
class ListItemPresenter(NodePresenter):
    ...
    def __init__(self, node: Node, depth: int = 0):
        ...
        cur_node_type = None
        for child in self._node.child_nodes:
            if child.type == NodeType.BULLET_LIST:
                self._child_presenters.append(BulletListPresenter(child, self._depth + 1))
            elif child.type == NodeType.ORDERED_LIST:
                self._child_presenters.append(OrderedListPresenter(child, self._depth + 1))
            else:
                child_presenter = create_node_presenter_from_node(child, idx == 0, cur_node_type == NodeType.HARD_BREAK,
                                                                  self._node)
                if child_presenter:
    ...
    def __str__(self):
        parts = list()
        for child_presenter in self._child_presenters:
            text = str(child_presenter)
            if isinstance(child_presenter, (BulletListPresenter, OrderedListPresenter)):
                parts.append('\n' + text)
            else:
                parts.append(text)
        return ''.join(parts)
...
def create_node_presenter_from_node(node: Node, is_first: bool, is_prev_hard_break: bool,
    ...
    if node.type == NodeType.PARAGRAPH:
        ...
        return OrderedListPresenter(node)
