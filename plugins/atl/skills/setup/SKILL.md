---
name: setup
description: 'Set up Atlassian CLI (acli) for Windows or Linux. Installs acli, configures ACLI_API_TOKEN, ACLI_EMAIL, and ACLI_SITE, and authorizes with Confluence and Jira. Use when acli is not installed, needs to be configured, or when mention "setup acli".'
---
...
```
If this succeeds, setup is complete. If it fails, check that `ACLI_API_TOKEN`, `ACLI_EMAIL`, and `ACLI_SITE` are set and re-run Steps 5–6.
