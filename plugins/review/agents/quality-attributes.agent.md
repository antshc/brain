...
```json
{
  ...
  "LABEL": "<confirmed issue severe enough it must be fixed before merge → `blocking`; other confirmed issue → `bug`; plausible risk → `suggest`; no issue found → not emitted>",
