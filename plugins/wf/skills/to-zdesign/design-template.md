...
<!-- Optional. Include only when the user explicitly asks for a Solution Overview flow diagram and ownership by container or component is itself a design decision. Follow `/behavior-diagram` skill **Swimlane Diagram** in current mode; that skill owns the template read. Do not compose a Mermaid skeleton from this template. -->
