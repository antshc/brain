...
- User must authenticate with `acli` before running this skill, using `echo "<api_token>" | acli jira auth login --token --email user@hpe.com --site zerto.atlassian.net` with `Api token` and following the prompts to connect to the Confluence instance.
