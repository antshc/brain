...
- `**Behaviour changes**` — system changes not visible in the delta lines themselves (e.g. a default list order set server-side, a new exception type thrown) — one line per item, `+|~|-` prefix; nested inside its own resource's `<details>`, not a separate top-level section; omit entirely when the resource has no such change.
