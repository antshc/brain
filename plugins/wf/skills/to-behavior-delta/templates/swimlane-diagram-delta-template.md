...
<!-- Include only when "who owns this step" (which container, or which component/module inside one container) is itself a design decision for this capability — not merely step order. If ownership doesn't matter, use the flowchart delta instead; if the focus is messages over time between participants, use the sequence diagram delta instead. Delete this instruction. -->

<!-- This diagram shows the flow DELTA for this capability, same convention as the flowchart delta: mark a node added (`:::added`) or removed (`:::removed`). There is no `:::memberChanged` equivalent — a materially changed node is either restyled `:::added` or left unstyled and described in prose. A lane that is itself entirely new or removed for this capability has no supported Mermaid styling mechanism (no border-color hook on `subgraph`) — call it out in prose instead. Omit unchanged nodes and lanes not needed to connect the delta. Delete this instruction. -->
...
<!-- Mermaid technical gotchas and good practices (mermaid.ai/open-source/syntax/swimlanes.html#good-practices), consistent with the flowchart delta conventions:
...
### Level 1 — Container Swimlane: {{capabilityTitle}}

<!-- Lanes are containers (the deployable/runnable units from the Solution Diagram — GUI, REST API service, database, queue). Show which container performs each step of this capability's flow. Delete this instruction. -->

<details>
<summary>{{capabilityTitle}} — container swimlane</summary>
...
swimlane-beta TB
  ...
  subgraph {{ownerContainerLane}} [{{capabilityOwnerContainerName}}]
    ...
    {{newStep}}[{{newComponentStep}}]:::added
    {{removedStep}}[{{deletedComponentStep}}]:::removed
  end
  ...
  {{decisionNode}} -->|{{noOutcome}}| {{removedStep}}
  {{decisionNode}} -->|{{yesOutcome}}| {{newStep}}
  {{newStep}} -->|{{handoff3}}| {{persistStep}}
...
### Level 2 — Component Swimlane: {{capabilityTitle}} inside {{containerName}}

<!-- Scoped to one container from Level 1. Lanes are the components/modules inside that container (e.g. GUI pages, REST API controllers/modules). Show which component performs each step of this capability's flow inside that container. Include only when internal component wiring, not just the container-level flow, is a design decision. Delete this instruction. -->

<details>
<summary>{{capabilityTitle}} — component swimlane ({{containerName}})</summary>
...
swimlane-beta TB
  ...
  subgraph {{ownerComponentLane}} [{{capabilityOwnerComponentName}}]
    ...
    {{newModuleStep}}[{{newModule}}]:::added
    {{removedModuleStep}}[{{deletedModule}}]:::removed
  end
  ...
  {{validateNode}} -->|{{noOutcome}}| {{removedModuleStep}}
  {{validateNode}} -->|{{yesOutcome}}| {{newModuleStep}}
  {{newModuleStep}} -->|{{handoff2}}| {{delegateStep}}
...
<!-- `added` = solid green border, new node. `removed` = solid red border, deleted node. Delete unused example lanes/nodes/classDefs. Include only the level(s) that are decision-relevant for this capability. -->
