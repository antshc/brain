---
name: manage-docs
description: Create and update the project's domain-model documentation — the glossary (CONTEXT.md), the architecture map (ARCHITECTURE.md), and the decision records (SSRs and ADRs). Owns the doc templates and the rules for where each file lives, when to create it, and how to keep the indexes in sync. Called by the grill-* skills to set up and update docs as decisions crystallise.
...
- `ARCHITECTURE.md` — the structural map, and the index of SSRs and ADRs. Use [ARCHITECTURE-FORMAT.md](./ARCHITECTURE-FORMAT.md).
- `docs/ssr/` — Solution Strategy Records: the backbone rules. Use [SSR-FORMAT.md](./SSR-FORMAT.md).
- `docs/adr/` — Architecture Decision Records: localized decisions. Use [ADR-FORMAT.md](./ADR-FORMAT.md).

## Where the files live

Read [FILE-STRUCTURE.md](./FILE-STRUCTURE.md) once when setting up the domain-model files for a repo that doesn't have them yet, or when deciding where a new file belongs. It covers single- vs multi-context repos (`CONTEXT-MAP.md`) and the lazy-creation rules.
...
- No `docs/ssr/` → create it when the first SSR is needed, then add it to the `## Solution Strategy` index in `ARCHITECTURE.md`.
...
qualifies as an SSR or ADR, write the record and update the matching index in the same change.
...
- `ARCHITECTURE.md` describes **shape and rules**, not implementation detail. Not a spec, not a scratch pad, not a place to inline backbone decisions — the step-by-step detail lives in the code and in the linked SSRs.

## Keeping the indexes in sync

`ARCHITECTURE.md` is the entry point a reader (or agent) scans before designing. Every record in
`docs/ssr/` and `docs/adr/` must appear in its index table with a matching summary; nothing is
...
When the docs live in a dedicated documentation/context repository (separate from the codebase), seed its `copilot-instructions.md` from [instructions.template](./instructions.template). It defines where `CONTEXT.md`, `ARCHITECTURE.md`, `docs/adr/`, and `docs/ssr/` live at the reporoot and where
the source code and worktrees live (`workspace/`).
