...
How to split requirements into **capabilities** before writing requirement sets. A **capability** is behavior the system provides *independently of where it appears*, and it *survives after the current change is completed* — never a one-off task. Each capability becomes exactly one requirement set.
...
1. **Is this a stable behavior domain?** If the candidate names placement or a surface ("show notification count in header"), rewrite it as pure behavior ("provide an active notification summary") before judging. Done when the candidate names a behavior with no placement left in it.
...
## Worked example
*"Show notification count in header"* is not yet a capability — it carries placement. Rewrite the behavior as *"Provide an active notification summary"*, yielding the capability `notification-summary`. If notification-count rules can change without order-action rules changing, keep `notification-summary` separate from the order capability.
