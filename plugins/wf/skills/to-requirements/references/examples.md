...
- Let an administrator review the files available for recovery.
- Restore a selected file to its original location.
- Require the administrator to choose a new location when the original location no longer exists.
- Prevent users without restore permission from restoring files.
...
### Functional Requirements
- Retrieve current availability for each item shown on the catalog page.
- Display an availability status beside every visible item.
- Keep rendering the catalog when availability retrieval partially fails.
