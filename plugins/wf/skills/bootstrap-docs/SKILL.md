---
name: bootstrap-docs
description: Own the existence-and-creation mechanics for ARCHITECTURE.md and CONTEXT.md — mandatorily create either the moment it's missing. Owns no templates, no prose, no section insertion (that's index-docs), no directory creation for docs/concepts or docs/services (each owned by the skill that writes into it). Invoked only when a caller's own existence check finds ARCHITECTURE.md or CONTEXT.md missing.
---
