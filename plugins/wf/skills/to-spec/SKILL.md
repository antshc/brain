...
3. Write the spec using the template below. If the feature changes an API, Database, or Resource contract, Run `/contract-delta` skill once per touched contract kind and inline its output verbatim under **Contracts Delta**. Then Run `/manage-backlog` skill **Publish spec** to publish it to the project ticket tracker. Apply the `spec` triage label - no need for additional triage.
...
Run `/contract-delta` skill once per touched contract kind (API, Database, Resource) and inline its output verbatim here, one block per kind.
