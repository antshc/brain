...
**Rules:** Add a row only when the framed outcome depends on a deployable's internal decision and its proved boundary contracts do not explain that decision. This is deliberately deferred scope, not missing chain evidence. `Capability question` is the verbatim domain-facing frame for an optional Capability-axis run; its inbound contract becomes the entry point and its expected outbound contracts bound the observable outcomes. Omit this section when no lane qualifies.

| Lane | Rationale | Capability question | Inbound contract | Expected outbound contracts |
|---|---|---|---|---|
