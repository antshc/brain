...
- **Context and decisions live at the reporoot:**
  ...
  - Solution Design Records (SDRs) under `docs/sdr/` — the architectural backbone; broader than ADRs, which record localized decisions. Indexed in `ARCHITECTURE.md`.
...
- **Author documentation at the reporoot** — `CONTEXT.md`, `CONTEXT-MAP.md`, ADRs under `docs/adr/` and SDRs under `docs/sdr/` are created and updated here, never inside `workspace/`.
