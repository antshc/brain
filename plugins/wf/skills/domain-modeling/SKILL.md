...
3. **Match Concepts/ADRs via `/trigger-indexer` Scan and match** — indexing alone never implies relevance; delegate the mechanical Trigger-condition matching to `/trigger-indexer` **Scan and match**, passing `ARCHITECTURE.md`'s `Crosscutting Concepts`/`Architecture Decision Records` sections and the current change's touched surface (entity/data shape, endpoint, folder, change type). Apply the same call to a Building-block service's full doc if one is linked (`BUILDING-BLOCK-SERVICE-FORMAT.md`) — unlinked services are trivial, the one-line bullet suffices. Log every returned verdict in the session ledger (*Track opened records*): matches as `{{n}} — opened, trigger matched: "{{clause}}"`; non-matches as `{{n}} — skipped, checked "{{clause1}}", "{{clause2}}": neither touched`.
