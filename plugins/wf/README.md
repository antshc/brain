...
4. **Story slicing** — `/to-stories` breaks the feature design down into one atomic story per capability, running [`/draft-story`](skills/draft-story/SKILL.md) to write each one; `/draft-story` also runs on its own whenever a single user story is asked for.
